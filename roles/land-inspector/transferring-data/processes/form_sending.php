<?php
    ...
    // Marcado del formulario como finalizado
    $query = "UPDATE data_trans_buque
        SET finalizado = ?
        WHERE id_data_trans = ?";
    
    $stat = $connection->prepare($query);
    $stat->bind_param('ii', $finalizado, $id_data_trans);
    
    if ($stat->execute()) {
        // Borrado de datos almacenados en el formulario (para un nuevo formulario)
        if (!$existente) {
            /* echo '<script>
        ...
        // Redirección al formulario terminado
        echo '<script>
            form.action = "../finished_form.php";
            form.submit();
        </script>';
    } else {
        if ($existente) {
            echo '<script>
                form.action = "../unfinished_form.php?form_error=sending_error";
                form.submit();
            </script>';
        } else {
            echo '<script>
                form.action = "../new_form.php?form_error=sending_error";
                form.submit();
            </script>';
        }
    }
